...
- Observability improvements to doc-update workflow: reasoning displayed in GitHub Actions job summary via `display_report: true` for transparent automated documentation decisions.
- CI/CD architecture documentation at `docs/architecture/ci-cd.md` covering the doc-update workflow's configuration, tool permissions, safety guards, and troubleshooting.
- Party mode easter egg: library card barcode on settings page is a hidden clickable toggle. Subtle gold glow hint and faster scan animation appear on hover to aid discovery. Click activates party mode with cycling rainbow borders, purple scan line animations, audio, and localStorage persistence.
...
- Daily streak bonus increased from +15 XP to +20 XP per day.
...
- Documentation update workflow tool permissions resolved through iterative fixes for Bash command patterns.

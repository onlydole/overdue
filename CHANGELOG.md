...
- Tests, lint and types CI workflow (`.github/workflows/checks.yml`): three parallel jobs running `pytest`, `ruff check` + `ruff format --check`, and `mypy` strict on every pull request and push to `main`, so the checks CONTRIBUTING.md documents are actually enforced. Uses no secrets and `contents: read` only, so it runs on fork PRs. CI pins Python 3.12 while the Dockerfile ships 3.14 (PR #118).

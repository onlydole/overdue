...
### Added
- Documentation freshness pipeline: `.github/scripts/freshness.py` scores every doc page on a 0--100 scale from three deterministic signals (git age delta, frontmatter TTL, symbol drift). The `freshness.yml` workflow gates merges on an SLO (median < 75 or any `critical: true` page < 60 fails), posts a sticky per-page delta comment, and routes 35--64 gray-zone pages through a conditional Claude semantic check. `freshness-badge.yml` publishes a Shields.io endpoint badge to `gh-pages` on each push to `main`, surfaced in the README. Each living doc declares its `ttl_days` and source globs in frontmatter; `installation.md`, `quickstart.md`, and `authentication.md` are tagged `critical: true`. Token noise is filtered via a camel-case-or-period regex plus an fnmatch allowlist at `.github/freshness-allowlist.txt` (PR #69).

...
Overdue includes a GitHub Actions workflow (`.github/workflows/doc-update.yml`) that automatically detects when merged PRs introduce documentation drift and proposes updates via follow-up PRs.

### How It Works

When a PR is merged to `main`, the workflow:

1. Detects which files changed in the merged PR
2. Uses [Claude Code Action](https://github.com/anthropics/claude-code-action) to analyze the changes against existing documentation
3. Determines if documentation updates are needed (skips internal refactors, bug fixes matching docs, and perf-only changes)
4. If updates are needed, creates a new branch (`docs/update-from-pr-{number}`) and opens a follow-up PR with the changes

The workflow triggers on all merged PRs to ensure documentation stays current even when indirect changes might affect docs (e.g., UI changes, workflow modifications, template updates, etc.).

### Setup Requirements

To enable this workflow in your repository:

1. **Add repository secret**: `ANTHROPIC_API_KEY` containing your Anthropic API key
2. **Install the Claude GitHub App**: [https://github.com/apps/claude](https://github.com/apps/claude)
3. **Customize CLAUDE.md**: This file in the repo root provides context and conventions to Claude Code
4. **Configure workflow permissions**: The `update-docs` job requires `id-token: write` permission for OIDC authentication with Anthropic's API via `claude-code-action@v1`. Without this permission, the action will fail with an OIDC token error after 3 retries. Ensure your workflow includes:
   ```yaml
   permissions:
     contents: write
     pull-requests: write
...
Full documentation: [https://code.claude.com/docs/en/github-actions](https://code.claude.com/docs/en/github-actions)
